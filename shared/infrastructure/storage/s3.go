...
func NewS3Storage(cfg *config.StorageConfig, obs ports.Observability) (ports.Storage, error) {
	logger, metrics, err := obs.ComponentsScoped("storage.s3")
	if err != nil {
		logger.Error("Failed to create s3 storage", err)
...
func newClientBuilder(cfg *config.StorageConfig, logger ports.Logger, metrics ports.Metrics) *clientBuilder {
...
func (c *Client) Put(ctx context.Context, bucket, key string, reader io.Reader, metadata ports.ObjectMetadata) error {
...
func (c *Client) GetWithMetadata(ctx context.Context, bucket, key string) (io.ReadCloser, *ports.ObjectMetadata, error) {
...
func (c *Client) List(ctx context.Context, bucket, prefix string) ([]ports.ObjectInfo, error) {
...
func (c *Client) newPutOperation(bucket, key string, reader io.Reader, metadata ports.ObjectMetadata) *putOperation {
	return &putOperation{
		client:   c,
...
func (op *getWithMetadataOperation) Execute(ctx context.Context) (io.ReadCloser, *ports.ObjectMetadata, error) {
	timer := op.client.startTimer()
	...
	if err != nil {
		if isNotFoundError(err) {
			...
			return nil, nil, ports.ErrObjectNotFound
		}
		op.recordError(err)
...
func (op *getWithMetadataOperation) extractMetadata(result *s3.GetObjectOutput) *ports.ObjectMetadata {
...
func (op *listOperation) Execute(ctx context.Context) ([]ports.ObjectInfo, error) {
	timer := op.client.startTimer()
...
func (op *listOperation) fetchAllObjects(ctx context.Context) ([]ports.ObjectInfo, int, error) {
	input := op.buildInput()
...
func (op *listOperation) extractObjects(page *s3.ListObjectsV2Output) []ports.ObjectInfo {
	objects := make([]ports.ObjectInfo, 0, len(page.Contents))
